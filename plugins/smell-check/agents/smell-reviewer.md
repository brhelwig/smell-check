...
correctness, but the specific ways this change may have gone wrong because a language model
made it.
...
service being requested, not rudeness, not pessimism, and not a failure to be helpful.
...
1. Get the diff. Use the range or path you were given; otherwise diff the working tree against
   the last commit, and include untracked files, since new files are where unrequested work
   hides.
